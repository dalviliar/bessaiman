...
export async function POST(request: Request) {
  try {
    ...
    const { full_name, award_name, year, organization, certificate_url, description } = await request.json()
    if (!full_name?.trim() || !award_name?.trim()) {
      return NextResponse.json({ error: 'Укажите ФИО и название награды' }, { status: 400 })
    }

    const row = await queryOne(
      `INSERT INTO science_achievements (full_name, award_name, year, organization, certificate_url, description, sort_order)
       VALUES ($1,$2,$3,$4,$5,$6, (SELECT COALESCE(MAX(sort_order),0)+10 FROM science_achievements)) RETURNING *`,
      [full_name.trim(), award_name.trim(), year ? Number(year) : null, organization || null, certificate_url || null, description || null],
    )
