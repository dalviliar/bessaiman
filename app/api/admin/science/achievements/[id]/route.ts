...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { full_name, award_name, year, organization, certificate_url, sort_order } = await request.json()
    if (!full_name?.trim() || !award_name?.trim()) {
      return NextResponse.json({ error: 'Укажите ФИО и название награды' }, { status: 400 })
    }

    const row = await queryOne(
      `UPDATE science_achievements SET full_name=$1, award_name=$2, year=$3, organization=$4, certificate_url=$5, sort_order=$6
       WHERE id=$7 RETURNING *`,
      [full_name.trim(), award_name.trim(), year ? Number(year) : null, organization || null, certificate_url || null, sort_order ?? 0, id],
    )
