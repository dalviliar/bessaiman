...
function EditUserModal({ user, onClose, onSaved }: { user: AdminUser; onClose: () => void; onSaved: () => void }) {
  ...
  const isSystemRole = (user.role as AdminRole)?.is_system ?? true
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!isSystemRole) body.permissions = { ...existingPerms, ...perms }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-2xl p-6 max-h-[90vh] overflow-y-auto"
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-base font-bold text-white">Редактировать пользователя</h2>
            <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>{user.email}</p>
          </div>
          <button onClick={onClose} style={{ color: 'rgba(255,255,255,0.4)' }}><X size={18} /></button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs mb-1.5" style={{ color: 'rgba(255,255,255,0.5)' }}>
              Новый пароль <span style={{ color: 'rgba(255,255,255,0.25)' }}>(оставьте пустым, чтобы не менять)</span>
            </label>
            <div className="relative">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={handleSave} disabled={loading}
              className="flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50"
              style={{ background: 'linear-gradient(135deg,#1D4ED8,#3B82F6)', color: 'white' }}>
              {loading ? <><Loader2 size={14} className="animate-spin" /> Сохраняем...</> : 'Сохранить'}
            </button>
            <button onClick={onClose} className="btn-secondary px-4 text-sm">Отмена</button>
          </div>
        </div>
      </div>
    </div>
                        ...
                        <p className="font-medium text-white text-sm">
                          ...
                          {isMe && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded"
                              style={{ background: 'rgba(59,130,246,0.15)', color: '#60A5FA' }}>Вы</span>
                          )}
                        </p>
