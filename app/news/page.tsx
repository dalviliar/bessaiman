    ...
    <div style={{ background: '#F8FAFC', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #E2E8F0' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex items-center gap-3">
          <Link href="/" className="text-sm font-medium" style={{ color: '#94A3B8' }}>{tr.news.backHome}</Link>
          <span style={{ color: '#E2E8F0' }}>/</span>
          <span className="text-sm font-semibold" style={{ color: '#0F172A' }}>{tr.news.breadcrumb}</span>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        <div className="mb-8">
          <div className="text-[12px] font-black tracking-widest mb-1" style={{ color: '#94A3B8' }}>BES SAIMAN GROUP</div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20" style={{ color: '#94A3B8' }}>
            ...
            <p className="text-base">{tr.news.emptyDesc}</p>
          </div>
        ) : (
            ...
            {posts.map(post => (
              <button key={post.id}
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <h2 className="font-bold text-lg leading-snug mb-2" style={{ color: '#0F172A' }}>
                    {postTitle(post)}
                  </h2>
                  {postContent(post) && (
                    <p className="text-base leading-relaxed flex-1 line-clamp-3" style={{ color: '#64748B' }}>
                      {postContent(post)}
                    </p>
                  )}
                  <div className="mt-3 text-sm font-semibold" style={{ color: '#1565C0' }}>
                    {tr.news.readMore}
                  </div>
                </div>
              </button>
